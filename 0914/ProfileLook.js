...
const pageCount = TotalCount > 0 ? Math.ceil(TotalCount / postsPerPage) : 0;


const handlePageClick = ({ selected }) => {
  const newPage = selected + 1;
  console.log('selected : ', selected); // 2페이지 = 1
  setCurrentPage(newPage); // 페이지 번호를 0에서 1로 바꿔서 전달
  console.log('current', newPage);
  fetchPosts(emailId, newPage, postsPerPage); // 페이지 번호는 1부터 시작
};
...
// // 다음페이지
 //const handleGoToNextPage = (newPageNumber) => {
   ...
   //const newPageNumber = pageNumber + 1;
  // const newOffset = newPageNumber * limit;
  // setPageNumber(newPageNumber);
  // setOffset(newOffset);
 //};
