...
app.post('/api/post', upload.single('image'), (req, res) => {
  ...
  //const imageUrl = encodeURIComponent(`http://localhost:4000/uploads/${req.file.filename}`);
      ...
      const query = 'INSERT INTO your_post (title, description, category, image_url) VALUES (?, ?, ?, ?)';
      const values = [data.title, data.description, data.category, imageUrl];
...
app.get('/api/home/:category', (req, res) => {
    ...
    const query = `SELECT id,image_url FROM your_post WHERE category = ? ORDER BY id DESC LIMIT ? OFFSET ?  `;
...
app.put('/api/postedit/:id', upload.single('newImageFile'), (req, res) => {
  ...
  const { title, description, category} = data.updatedData;
  let imageUrl = data.updatedData.image_url; 
