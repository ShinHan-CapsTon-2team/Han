      ...
      try {
        ...
        if (response.ok) {
          ...
          //console.log('post after setPost:', post);
           // 영어 카테고리를 한글로 변환
          const translatedCategory = categoryLabels[postData.category];
          postData.category = translatedCategory || postData.category; // 변환된 값이 없으면 원래 값 유지

          setUpdatedPost(postData);
          setPost(postData);
          console.log('Updated post data:', postData);
          console.log("변환된 값:",translatedCategory)
        } else {
